...
{
    public class HistorialCasoContenciosoDataAccess
    {
        private readonly string _apiUrl = "http://bpa.com.es/peticiones-litigios/contenciosos/historial_caso_contencioso.php";

        private static readonly HttpClient _http = new HttpClient();
        public async Task<ApiResponseHistorialCasoContencioso> ListarHistorialCasoContencioso(int casoId)
        {
            var parameters = new Dictionary<string, string>
            {
                { "action", "listar_historial_caso_contencioso" },
            ...
            try
            {
                ...
                return JsonConvert.DeserializeObject<ApiResponseHistorialCasoContencioso>(jsonResult)
                       ?? new ApiResponseHistorialCasoContencioso { success = false, message = "Respuesta vacía o inválida." };
            }
            catch (Exception ex)
            {
        ...
        public async Task<ApiResponse<object>> EditarHistorialCasoContencioso(EditarHistorialCasoContenciosoRequest req)
        {
            var parameters = new Dictionary<string, string>
            {
                { "action", "editar_historial_caso_contencioso" },
        ...
        public async Task<ApiResponse<object>> EliminarHistorialCasoContencioso(int historialId, int casoId, int usuarioId)
        {
            var parameters = new Dictionary<string, string>
            {
                { "action", "eliminar_historial_caso_contencioso" },
        ...
        string origen)
        {
            var parameters = new Dictionary<string, string>
            {
                { "action", "terminar_caso_contencioso" },
                { "caso_id", casoId.ToString() },
                { "usuario_id", usuarioId.ToString() },
