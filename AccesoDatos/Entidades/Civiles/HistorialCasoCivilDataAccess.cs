...
{
    public class HistorialCasoCivilDataAccess
    {
        private readonly string _apiUrl = "http://bpa.com.es/peticiones-litigios/civiles/historial_caso_civil.php";

        private static readonly HttpClient _http = new HttpClient();
        public async Task<ApiResponseHistorialCasoCivil> ListarHistorialCasoCivil(int casoId)
        {
            var parameters = new Dictionary<string, string>
            {
                { "action", "listar_historial_caso_civil" },
            ...
            try
            {
                ...
                return JsonConvert.DeserializeObject<ApiResponseHistorialCasoCivil>(jsonResult)
                       ?? new ApiResponseHistorialCasoCivil { success = false, message = "Respuesta vacía o inválida." };
            }
            catch (Exception ex)
            {
        ...
        public async Task<ApiResponse<object>> EditarHistorialCasoCivil(EditarHistorialCasoCivilRequest req)
        {
            var parameters = new Dictionary<string, string>
            {
                { "action", "editar_historial_caso_civil" },
        ...
        public async Task<ApiResponse<object>> EliminarHistorialCasoCivil(int historialId, int casoId, int usuarioId)
        {
            var parameters = new Dictionary<string, string>
            {
                { "action", "eliminar_historial_caso_civil" },
