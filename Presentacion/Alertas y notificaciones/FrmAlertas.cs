...
namespace Presentacion.Alertas_y_notificaciones
{
    public partial class FrmAlertas : Form
    {
        ...
        private readonly AlertasModel alertasModel = new AlertasModel();
        ...
        private void CrearBotonesAccion(DataGridView dtg)
        {
            ...
            // Editar
            if (!dtg.Columns.Contains("MarcarComoLeida"))
            {
            ...
            dtg.Columns["MarcarComoLeida"].DisplayIndex = dtg.ColumnCount - 1;
        ...
        private async Task CargarAlertas()
        {
            ...
            switch (modulo)
            {
                ...
                case "Contencioso Administrativo":
                    {
                        moduloSeleccionado = 4;
                        break;
                    }
                case "Constitucional":
                    {
                        moduloSeleccionado = 3;
                        break;
                    }
            ...
            var response = await alertasModel.ObtenerAlertasUsuarioPaginadas(
                usuarioId: usuarioId,
                moduloId: moduloSeleccionado,
                pagina: paginaActual,
                registrosPorPagina: registrosPorPagina,
                soloNoLeidas: checkBoxSoloNoLeidas.Checked,
            ...
            if (response.success)
            {
                ...
                labelTotal.Text = $"Total de alertas: {totalRegistros}";
        ...
        private async void FrmAlertas_Load(object sender, EventArgs e)
        {
            // Asignar BindingSource al DataGridView
            dtgAlertas.DataSource = bsAlertas;
            comboBoxRama.SelectedIndex = 4;
            await CargarAlertas();
            dtgAlertas.Columns["MarcarComoLeida"].Width = 40;
        ...
        {
            if (dtgAlertas.Columns[e.ColumnIndex].Name == "Nombre" && e.Value != null)
            {
                ...
                dtgAlertas.Rows[e.RowIndex].Cells[e.ColumnIndex].ToolTipText = iniciales;
            }
        ...
        private void FrmAlertas_Resize(object sender, EventArgs e)
        {
            CentrarPanel();
        }
        ...
        private async void btnEliminarAlertasAntiguas_Click(object sender, EventArgs e)
        {
            try
            {
                int usuarioId = UserSession.Id;
                ...
                var confirmacion = MessageBox.Show(
                    "Esta acción eliminará las alertas antiguas ya leídas y no se podrá deshacer. ¿Desea continuar?",
                ...
                var response = await alertasModel.EliminarAlertasAntiguas(
                    usuarioId: usuarioId,
                    moduloId: moduloSeleccionado,
                    soloLeidas: true
                );
                ...
                {
                    MessageBox.Show(
                        $"Se eliminaron {response.eliminadas} alertas antiguas.",
                        "Proceso completado",
            ...
            {
                MessageBox.Show(
                    "Error al eliminar alertas antiguas: " + ex.Message,
                    "Error",
                    MessageBoxButtons.OK,
                    MessageBoxIcon.Error
                );
        ...
        private async void dtgAlertas_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (e.RowIndex < 0) return;

                if (dtgAlertas.Columns[e.ColumnIndex].Name == "MarcarComoLeida")
                {
                    int alertaId = Convert.ToInt32(dtgAlertas.Rows[e.RowIndex].Cells["id"].Value);
                    int usuarioId = UserSession.Id;

                    DialogResult confirmacion = MessageBox.Show(
                        "¿Desea marcar esta alerta como leída?",
                        "Confirmar acción",
                        MessageBoxButtons.YesNo,
                    ...
                    var response = await alertasModel.MarcarAlertaLeida(alertaId, usuarioId);

                    if (response.success)
                    {
                        MessageBox.Show(
                            response.message ?? "La alerta fue marcada como leída.",
                            "Información",
                            MessageBoxButtons.OK,
                            MessageBoxIcon.Information
                        );
                    ...
                    {
                        MessageBox.Show(
                            response.message ?? "No fue posible marcar la alerta como leída.",
                            "Error",
                            MessageBoxButtons.OK,
                            MessageBoxIcon.Error
                        );
            ...
            {
                MessageBox.Show(
                    "Error al marcar la alerta como leída: " + ex.Message,
                    "Error",
                    MessageBoxButtons.OK,
                    MessageBoxIcon.Error
                );
