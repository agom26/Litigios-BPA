...
{
    public partial class BuscarAbogadoForm : Form
    {
        ...
        public BuscarAbogadoForm(BindingList<UserListDataResponse> listaDestino, string tipoPersona)
        {
            InitializeComponent();
            switch (tipoPersona)
            {
                case "Abogado Director":
                    {
                        comboBoxRama.Items.Clear();
                        comboBoxRama.Items.AddRange(new string[] { "Abogado Director" });
                        comboBoxRama.SelectedIndex = 0;
                        break;
                    }
                case "Abogado Asistente":
                    {
                        comboBoxRama.Items.Clear();
                        comboBoxRama.Items.AddRange(new string[] { "Abogado Asistente" });
                        comboBoxRama.SelectedIndex = 0;
                        break;
                    }
                case "Socio Responsable":
                    {
                        comboBoxRama.Items.Clear();
                        comboBoxRama.Items.AddRange(new string[] { "Socio Responsable" });
                        comboBoxRama.SelectedIndex = 0;
                        break;
                    }
                default:
                    {
                        ...
                        comboBoxRama.Items.AddRange(new string[] { "Abogado Director", "Abogado Asistente", "Socio Responsable" });
                        comboBoxRama.SelectedIndex = 0;
        ...
        private void dtgSociosResponsables_DataBindingComplete(object sender, DataGridViewBindingCompleteEventArgs e)
        {
            ...
            if (dtgAbogados.Columns["id"] != null)
            {
                dtgAbogados.Columns["id"].Visible = false;
            }

            dtgAbogados.ClearSelection();
        }
        ...
        {
            if (dtgAbogados.SelectedRows.Count > 0)
            {
                foreach (DataGridViewRow row in dtgAbogados.SelectedRows)
                {
        ...
        private void dtgSociosResponsables_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            int idPersona = Convert.ToInt32(dtgAbogados.Rows[e.RowIndex].Cells["id"].Value);
        }
        ...
        private async void BuscarAbogadoForm_Load(object sender, EventArgs e)
        {
            dtgAbogados.DataSource = bsSociosResponsables;
            await Filtrar();
        }
