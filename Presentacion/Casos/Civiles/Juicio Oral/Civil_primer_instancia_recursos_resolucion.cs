...
{
    public partial class Civil_primer_instancia_recursos_resolucion : Form, IAsyncLoadable
    {
        ...
        private int paginaActualNoDefinitivas = 1;
        private int registrosPorPaginaNoDefinitivas = 10;
        private int totalRegistrosNoDefinitivas = 0;

        private int paginaActualFinJuicio = 1;
        private int registrosPorPaginaFinJuicio = 10;
        private int totalRegistrosFinJuicio = 0;

        private HistorialCasoCivilDetalle _historialSeleccionado;
        ...
        CasoCivilJuicioOralRecursosModel casoCivilModel = new CasoCivilJuicioOralRecursosModel();
        ArchivosCivilesModel archivosModel = new ArchivosCivilesModel();
        ...
        public Civil_primer_instancia_recursos_resolucion()
        {
            InitializeComponent();
        }
        ...
        {
            txtExpediente.Enabled = !isLectorCivil;
            txtNombreParticular.Enabled = !isLectorCivil;
            comboboxNotificador.Enabled = !isLectorCivil;
            comboBoxJuzgado.Enabled = !isLectorCivil;
            comboboxOficial.Enabled = !isLectorCivil;

            btnAgregarDemandados.Enabled = !isLectorCivil;
            btnAgregarDemandantes.Enabled = !isLectorCivil;
            btnAgregarPartesInteresadas.Enabled = !isLectorCivil;
            btnAgregarContactoEmpresa.Enabled = !isLectorCivil;
            btnAgregarAbogadosAsistentes.Enabled = !isLectorCivil;
            btnAgregarAbogadosDirectores.Enabled = !isLectorCivil;
        ...
        private async Task CargarCasosNoDefinitivas()
        {
            ...
            var response = await casoCivilModel.ObtenerCasosCiviles(idUsuario, paginaActualNoDefinitivas, registrosPorPaginaNoDefinitivas, "no_definitiva", filtro);
            if (IsDisposed || !IsHandleCreated) return;
            ...
            if (response.success)
            {
                ...
                totalRegistrosNoDefinitivas = response.total;
                labelTotal.Text = $"Total de casos civiles: {totalRegistrosNoDefinitivas}";
                lblPagina.Text = $"Página {paginaActualNoDefinitivas} de {Math.Ceiling((double)totalRegistrosNoDefinitivas / registrosPorPaginaNoDefinitivas)}";
            }
            else
            {
                if (!this.IsDisposed)
        ...
        private async Task CargarCasosFinJuicio()
        {
            ...
            var response = await casoCivilModel.ObtenerCasosCiviles(idUsuario, paginaActualFinJuicio, registrosPorPaginaFinJuicio, "fin_juicio", filtro);
            if (IsDisposed || !IsHandleCreated) return;
            ...
            if (response.success)
            {
                ...
                // Actualizar paginación
                totalRegistrosFinJuicio = response.total;
                labelTotal2.Text = $"Total de casos civiles: {totalRegistrosFinJuicio}";
                lblPagina2.Text = $"Página {paginaActualFinJuicio} de {Math.Ceiling((double)totalRegistrosFinJuicio / registrosPorPaginaFinJuicio)}";
            }
            else
            {
                if (!this.IsDisposed)
        ...
        private async void Civil_primer_instancia_recursos_resolucion_Load(object sender, EventArgs e)
        {
            await VerificarTipoUsuario();
            if (IsDisposed || !IsHandleCreated) return;
        ...
        {
            if (paginaActualNoDefinitivas * registrosPorPaginaNoDefinitivas < totalRegistrosNoDefinitivas)
            {
                paginaActualNoDefinitivas++;
        ...
        {
            if (paginaActualNoDefinitivas > 1)
            {
                paginaActualNoDefinitivas--;
                await EjecutarConLoaderAsync(async () =>
                {
        ...
        private async void dtgCasosLaborales_CellClick(object sender, DataGridViewCellEventArgs e)
        {
                ...
                if (confirm == DialogResult.Yes)
                {
                    ...
                    if (EstadoCivil.estado != null)
                    {
                        var response = await historialModel.TerminarCasoCivil(
                            ...
                            origen: "CIVIL JUICIO ORAL PRIMER INSTANCIA"
                        );
            ...
            if (dtgResolucionesNoDefinitivas.Columns[e.ColumnIndex].Name == "Archivos")
            {
                try
                {
                    _cargandoCaso = true;
            ...
            if (dtgResolucionesNoDefinitivas.Columns[e.ColumnIndex].Name == "Editar")
            {
                try
                {
                    ...
                    lblTitulo.Text = "Editar Caso Civil";
        ...
        private void Civil_primer_instancia_recursos_resolucion_Resize_1(object sender, EventArgs e)
        {
            CentrarPanel();
        }
        ...
        private void btnAgregarEstado_Click(object sender, EventArgs e)
        {
            if (!isLectorCivil)
            {
                FrmAgregarEstadoCivilJORecursos frmAgregarEstado = new FrmAgregarEstadoCivilJORecursos();
                frmAgregarEstado.ShowDialog();

                if (EstadoCivil.estado != null)
                {
                    _huboCambioEstado = true;
                    txtEstado.Text = EstadoCivil.estado.ToString();
                    txtObservaciones.AppendText(Environment.NewLine + EstadoCivil.observaciones);
        ...
        private void Civil_primer_instancia_recursos_resolucion_ResizeEnd(object sender, EventArgs e)
        {
        ...
        private async void btnEditarCaso_Click(object sender, EventArgs e)
        {
            ...
            if (resultado.success)
            {
                MessageBox.Show("Caso civil actualizado correctamente.", "Éxito", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        {
            var datosHistorial = await historialModel.ObtenerHistorialCasoCivil(_idCasoEditar);
        ...
        {
            var res = await archivosModel.ListarArchivosCasoCivil(_idCasoEditar);
        ...
        {
            var resp = await archivosModel.ListarArchivosCasoCivil(_idCasoEditar);
            if (IsDisposed || !IsHandleCreated) return;
        ...
        private async void dtgArchivos_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            try
            {
                ...
                if (colName == "Abrir")
                {
                    ...
                    await EjecutarConLoaderAsync(async () =>
                    {
                        resp = await archivosModel.DescargarArchivoCasoCivil(_idCasoEditar, archivoId, tempFile);
                    });
                ...
                else if (colName == "Descargar")
                {
                    ...
                    await EjecutarConLoaderAsync(async () =>
                    {
                        resp = await archivosModel.DescargarArchivoCasoCivil(_idCasoEditar, archivoId, sfd.FileName);
                    });
                ...
                else if (colName == "Eliminar")
                {
                    if (!isLectorCivil)
                    {
                        ...
                        await EjecutarConLoaderAsync(async () =>
                        {
                            resp = await archivosModel.EliminarArchivoCasoCivil(_idCasoEditar, archivoId);
                        });
                        if (IsDisposed || !IsHandleCreated) return;
        ...
        private async void btnSubirArchivo_Click(object sender, EventArgs e)
        {
            if (!isLectorCivil)
            {
                ...
                ApiResponse<List<SubirArchivoCasoCivilData>> response = null;
                ...
                try
                {
                    ...
                    await EjecutarConLoaderAsync(async () =>
                    {
                        response = await archivosModel.SubirArchivosCasoCivil(_idCasoEditar, ofd.FileNames.ToList());
                    });
                    if (IsDisposed || !IsHandleCreated) return;
        ...
        private async void dtgHistorial_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (colName == "Eliminar")
            {
                if (!isLectorCivil)
                {
                    ...
                    await EjecutarConLoaderAsync(async () =>
                    {
                        resp = await historialModel.EliminarHistorialCasoCivil(
                            item.id,
                            item.caso_id,
                            UserSession.Id
                        );
        ...
        {
            comboboxEstado.Enabled = !isLectorCivil;
            dateTimePickerFechaEstado.Enabled = !isLectorCivil;
            txtObservacionesHistorial.Enabled = !isLectorCivil;
            dateTimePickerFechaVencimiento.Enabled = !isLectorCivil;
            dateTimePickerHoraVencimiento.Enabled = !isLectorCivil;
        }
        ...
        private async void btnGuardarEdicionHistorial_Click(object sender, EventArgs e)
        {
            if (!isLectorCivil)
            {
                ...
                var req = new EditarHistorialCasoCivilRequest
                {
        ...
        private void VerificarEstadoEditarHistorial(string origen)
        {
            ...
            bool requiere = EstadoCivilHelper.RequiereVencimiento(estado, origen);
        ...
        private void ActualizarObservacionEditarHistorial()
        {
            ...
            txtObservacionesHistorial.Text = EstadoCivilHelper.GenerarObservacion(
                fechaEstado,
                comboboxEstado.Text,
        ...
        private async void dtgResolucionesFinJuicio_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (dtgResolucionesFinJuicio.Columns[e.ColumnIndex].Name == "Archivos")
            {
                try
                {
                    _cargandoCaso = true;
        ...
        private async void btnAnteriorFinJuicio_Click(object sender, EventArgs e)
        {
            if (paginaActualFinJuicio > 1)
            {
                paginaActualFinJuicio--;
                await EjecutarConLoaderAsync(async () =>
                {
                    await CargarCasosFinJuicio();
                });
        ...
        private async void btnSiguienteFinJuicio_Click(object sender, EventArgs e)
        {
            if (paginaActualFinJuicio * registrosPorPaginaFinJuicio < totalRegistrosFinJuicio)
            {
                paginaActualFinJuicio++;
                await EjecutarConLoaderAsync(async () =>
                {
