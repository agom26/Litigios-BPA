    ...
    public partial class FrmAgregarCasoComun : Form
    {
        ...
        private void dtgContactoEmpresas_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (dtgContactoEmpresas.SelectedRows.Count > 0)
            {
                ...
                var caso = (CasoCivilListItem)row.DataBoundItem;

                IdCasoSeleccionado = caso.id; // 👈 guardas el ID

                
            }
            else
            {
                MessageBox.Show("Debe seleccionar un Caso Común para poder agregarlo",
        ...
        private async Task Filtrar()
        {
            ...
            var resultado = await comunModel.ObtenerCasosCiviles(pagina, pagina, registrosPorPagina, filtro);
            
            if (resultado.success)
            {
                bsViaAmpremio.DataSource = resultado.data;
                dtgContactoEmpresas.Refresh();
                labelTotal.Text = $"Total de casos civiles comunes: {resultado.total}";
        ...
        private async void FrmAgregarCasoComun_Load(object sender, EventArgs e)
        {
