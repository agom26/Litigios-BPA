...
{
    public partial class Contencioso_RecursoCasacion : Form, IAsyncLoadable
    {
        ...
        private string? expedienteAmparo = null;
        ...
        public Contencioso_RecursoCasacion()
        {
            InitializeComponent();
        }
        ...
        private void LimpiarFormularioMR()
        {
            ...
            txtExpedienteAmparo.Text= "";
            txtExpedienteReferencia.Text = "";
        ...
        {
            txtExpediente.Enabled = !isLectorContencioso;
            txtNombreParticular.Enabled = !isLectorContencioso;
            comboboxNotificador.Enabled = !isLectorContencioso;
            comboBoxJuzgado.Enabled = !isLectorContencioso;
            comboboxOficial.Enabled = !isLectorContencioso;
            btnAgregarDemandados.Enabled = !isLectorContencioso;
            btnAgregarDemandantes.Enabled = !isLectorContencioso;
            btnAgregarPartesInteresadas.Enabled = !isLectorContencioso;
            btnAgregarContactoEmpresa.Enabled = !isLectorContencioso;
            btnAgregarAbogadosAsistentes.Enabled = !isLectorContencioso;
            btnAgregarAbogadosDirectores.Enabled = !isLectorContencioso;
            btnAgregarSociosResponsables.Enabled = !isLectorContencioso;
            ...
            btnEliminarCasoReferencia.Enabled = !isLectorContencioso;
        }

        private async Task CargarDatosCaso(int idCaso)
        {
            int idUsuario = UserSession.Id;
            var resp = await recursoCasacionModel.ObtenerRecursoCasacionPorId(idUsuario, idCaso);
            ...
            var motivoCasacion = resp.data.motivo_casacion ?? "";
            ...
            if (data != null)
            {
                ...
                txtObservaciones.Text = (data.caso_origen.observaciones ?? "").Replace("\n", Environment.NewLine) + Environment.NewLine+ 
                    (data.caso.observaciones ?? "")
                    .Replace("\n", Environment.NewLine);
            }
            ...
            var casoReferencia = data.caso_origen ?? null;
            if(casoReferencia != null)
            {
                txtExpedienteReferencia.Text = casoReferencia.expediente;
                txtNombreParticularCasoReferencia.Text = casoReferencia.nombre_particular;
            }
            else
            {
                txtExpedienteReferencia.Text = "";
            ...
            txtExpedienteAmparo.Text = resp.data.expediente_amparo ?? "";
        ...
        private async Task CargarCasos()
        {
            ...
            var response = await recursoCasacionModel.ObtenerRecursosCasacion(idUsuario, paginaActual, registrosPorPagina, filtro);
        ...
        private async void Contencioso_RecursoCasacion_Load(object sender, EventArgs e)
        {
            await VerificarTipoUsuario();
            if (!_yaCargo)
                await LoadAsync();
        ...
        private async Task GuardarCaso()
        {
            ...
            var resultado = await recursoCasacionModel.CrearRecursoCasacion(req);
        ...
        private async void dtgCasosCiviles_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (dtgCasosContenciosos.Columns[e.ColumnIndex].Name == "Eliminar")
            {
                ...
                if (confirm == DialogResult.Yes)
                {
                    ...
                    await EjecutarConLoaderAsync(async () =>
                    {
                        resultado = await recursoCasacionModel.EliminarRecursoCasacion(idCaso, UserSession.Id);
                    });
        ...
        private void Contencioso_RecursoCasacion_Resize_1(object sender, EventArgs e)
        {
            CentrarPanel();
        }
        ...
        private void btnAgregarEstado_Click(object sender, EventArgs e)
        {
            if (!isLectorContencioso)
            {
                FrmAgregarEstadoContenciosoRC frmAgregarEstado = new FrmAgregarEstadoContenciosoRC();
                frmAgregarEstado.ShowDialog();
                ...
                if (EstadoContencioso.estado != null)
                {
                    ...
                    if (EstadoContencioso.estado == "Amparo")
                    {
                        expedienteAmparo = frmAgregarEstado.expedienteA;
                        txtExpedienteAmparo.Text = expedienteAmparo;
                    }
                    else
                    {
                        expedienteAmparo = null;
                        txtExpedienteAmparo.Text = "";
                    }
        ...
        private void Contencioso_RecursoCasacion_ResizeEnd(object sender, EventArgs e)
        {
        ...
        {
            comboboxEstado.Enabled = !isLectorContencioso;
            dateTimePickerFechaEstado.Enabled = !isLectorContencioso;
            txtObservacionesHistorial.Enabled = !isLectorContencioso;
            dateTimePickerFechaVencimiento.Enabled = !isLectorContencioso;
            dateTimePickerHoraVencimiento.Enabled = !isLectorContencioso;
        }
        private void CargarDatosHistorialEnTab(HistorialCasoContenciosoDetalle item)
        {
            ...
            comboboxEstado.SelectedItem = item.estado ?? "";
            string origen = item.origen;
            bool requiereVencimiento = EstadoContenciosoHelper.RequiereVencimientoSegundaInstancia(item.estado ?? "");
        ...
        private void VerificarEstadoEditarHistorial()
        {
            ...
            bool requiereVencimiento = EstadoContenciosoHelper.RequiereVencimientoSegundaInstancia(
                comboboxEstado.Text
            );
