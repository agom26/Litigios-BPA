...
{
    public partial class ContactoEmpresa : Form
    {

        private int paginaActual = 1;
        private int registrosPorPagina = 10;
        private int totalRegistros = 0;
        private BindingSource bsContactosEmpresa= new BindingSource();
        private int _idContactoEmpresaEditar;
        ...
        {
            var persona=await contactoEmpresaModel.ObtenerDetallesContactoEmpresaPorId(idPersona);
        ...
        {
            lblTitulo.Text = "Nuevo Contacto de Empresa";
        ...
        private async Task CargarContactosDeEmpresa()
        {
            
            var response = await contactoEmpresaModel.ObtenerContactosDeEmpresa(paginaActual, registrosPorPagina);

            if (response.success)
            {
                // Asignar los datos al BindingSource
                bsContactosEmpresa.DataSource = response.data;
                dtgDemandantes.Refresh();
                // Actualizar paginación
                totalRegistros = response.totalRegistros;
                labelTotal.Text = $"Total de Contactos de Empresa: {totalRegistros}";
        ...
        private async void ContactoEmpresa_Load(object sender, EventArgs e)
        {

            // Asignar BindingSource al DataGridView
            dtgDemandantes.DataSource = bsContactosEmpresa;

            // Cargar Demandados
            await CargarContactosDeEmpresa();
        ...
        private async Task Filtrar()
        {
            ...
            {
                bsContactosEmpresa.DataSource = resultado.data;  
                dtgDemandantes.Refresh();
        ...
        private async Task ActualizarContactoDeEmpresa()
        {
            var resultado = await contactoEmpresaModel.EditarContactoEmpresa(
                _idContactoEmpresaEditar,
                txtNombre.Text,
                txtDireccion.Text, 
                txtCorreo.Text,
                txtTelefono.Text,
            ...
            if (resultado.success)
            {
                MessageBox.Show("Datos del contacto de empresa actualizados correctamente",
        ...
        private async Task GuardarContactoDeEmpresa()
        {
            ...
            if (resultado.success)
            {
                MessageBox.Show("Contacto de empresa creado correctamente.", "Éxito", MessageBoxButtons.OK, MessageBoxIcon.Information);
                await CargarContactosDeEmpresa();
                LimpiarFormulario();
                AnadirTabPage(Listar);
                EliminarTabPage(Detalles);
        ...
        {
            if (lblTitulo.Text == "Nuevo Contacto de Empresa")
            {
                await GuardarContactoDeEmpresa();
            }
            else
            {
                await ActualizarContactoDeEmpresa();
            }
        }
        ...
        private async void dtgDemandantes_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (dtgDemandantes.Columns[e.ColumnIndex].Name == "Eliminar")
            {
                ...
                {
                    var resultado = await contactoEmpresaModel.EliminarContactoEmpresa(idDemandante);

                    if (resultado.success)
                    {
                        MessageBox.Show("Contacto de empresa eliminado correctamente.", "Éxito",
            ...
            if (dtgDemandantes.Columns[e.ColumnIndex].Name == "Editar")
            {
                ...
                lblTitulo.Text = "Editar Contacto de Empresa";
                int idPersona = Convert.ToInt32(dtgDemandantes.Rows[e.RowIndex].Cells["id"].Value);
                _idContactoEmpresaEditar= idPersona;
